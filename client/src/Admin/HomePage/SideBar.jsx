...
import {BiBuilding,BiAddToQueue, BiLogOut, BiUserCircle} from "react-icons/bi"
    ...
    <>
      <main className="relative lg:w-72 ">
        <section className="w-full h-full text-white bg-gray-700 p-2">
          <div className="font-medium w-full flex flex-row gap-3 bg-gray-500 px-2 py-1 rounded-sm">
            <img
          ...
          <div className="p- flex flex-col gap-0 my-4">
          {
            sideLables.map((items,index)=>{
                ...
                return(
                    <>
                    {lable ==="Logout"? (
                        <button className=" hover:bg-gray-500 flex gap-2 my-2 font-medium p-3 items-center">
                            <spam>{icon}</spam>
                            <spam>{lable}</spam>
                        </button>
                    ):(
                        <Link to={ref} className={` flex gap-2 my-2 font-medium p-3 items-center ${activeTab === index ?"bg-gray-500":"hover:bg-gray-500"}`} >
                            <spam>{icon}</spam>
                            <spam>{lable}</spam>
                        </Link>
                    )}
