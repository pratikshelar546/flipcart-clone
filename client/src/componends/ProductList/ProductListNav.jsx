  ...
  return (
    <>
      <main className='relative w-full '>
        <section className=' w-full text-black h-28 border-b flex justify-center space-x-0 align-middle items-center shadow-md'>
          <div className='flex  items-center justify-center' >
            {Product.map((data, key) => {
              return (
                <>
                  <div className='pl-5 py-3 pr-4 text-center flex flex-col justify-center align-middle'>
                    <div className='w-16 h-16 '>

                      <img src={data.image} alt={data.title} className='' />
                      
                    </div>
                    <h1 key={key}>{data.title}</h1>
                  </div>
                </>
              )
            })}
